globals [
  ...
  fires-put-out   ;; how many fires were put out by planes
]
...
to setup
  ...
  ;; Make firefighters planes
  create-planes num-planes [

    set shape "airplane" ;; use the airplane shape
    ;; Set if all the planes will start from the max x coordinate of the world or in random places
    ifelse distribute-planes-randomly?
    [
      ;;set the planes in random places
...
to go
  if not any? fires and not any? embers  ;; If there is no more fire, stops the simulation
    [ stop ]
  ask fires ;; Spread the fire
